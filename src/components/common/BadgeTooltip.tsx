...
export function BadgeTooltip({
  ...
  const next = badge.nextBadgeName && badge.nextRequirement
    ...
    : badge.familyKey && badge.currentProgress != null
      ...
      : "";
  const bingo = badge.bingoLineCounts
    ? ` Kumulative BINGO-Linien: ${badge.bingoLineCounts.bronze} Bronze, ${badge.bingoLineCounts.silver} Silber, ${badge.bingoLineCounts.gold} Gold.`
    : "";
  return (
    <AccessibleTooltip
      label={`${badge.name} · ${badgeTierLabel[badge.isSpecialEventBadge ? "special" : badge.tier]}`}
      description={`Freischaltbedingung: ${badge.requirement}. Freigeschaltet am ${new Date(badge.awardedAt).toLocaleDateString("de-DE")}.${source}${bingo} Seltenheit: ${badge.rarityPercent == null ? "noch nicht berechenbar" : `${badge.rarityPercent.toLocaleString("de-DE", { maximumFractionDigits: 1 })} % der Spieler`}.${next}`}
      className={className}
    />
  );
}
