  ...
  return (
    <div className={cn("grid grid-cols-2 gap-3 lg:grid-cols-3", compact && "lg:grid-cols-4")}>
      {badges.map((badge, index) => {
        ...
        return <AnimatedCard key={badge.key} delay={Math.min(index * 0.025, 0.2)} className={cn("relative overflow-hidden border p-3 text-center sm:p-5", featured && "sm:px-7 sm:py-8", mobileLimit != null && index >= mobileLimit && !mobileExpanded && "hidden sm:block", styles[displayTier], variantStyles[badge.designVariant])}>
