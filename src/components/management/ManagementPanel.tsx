    ...
    <section>
      <div className="panel p-7">
        <div className="flex items-center justify-between gap-4">
          ...
          {unlocked && <Button variant="ghost" onClick={lock}><LogOut className="size-4" /> Sperren</Button>}
        </div>
        {!unlocked ? (
          <form
          ...
          >
            <Input type="password" value={code} onChange={(event) => setCode(event.target.value)} placeholder="Code eingeben" autoComplete="off" />
            <Button type="submit"><Lock className="size-4" /> Öffnen</Button>
            {error && <p aria-live="assertive" className="self-center text-sm text-red-300">{error}</p>}
          </form>
        ) : (
            ...
            <section className="rounded-2xl border border-white/10 p-5">
              ...
              <div className="mt-4 max-h-96 space-y-2 overflow-y-auto">
                {state.attempts.map((attempt) => {
                  ...
                  return (
                    <button key={attempt.id} type="button" onClick={() => setEditing(attempt)} className="flex w-full items-center gap-4 rounded-xl bg-white/[0.03] p-3 text-left transition hover:bg-white/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400">
                      <span className="min-w-0 flex-1 truncate font-semibold">{player?.name ?? "Unbekannt"}</span>
                      <span className="text-sm text-white/50">{attempt.result === "dns" ? "DNS" : formatTime(attempt.timeSeconds ?? 0)}</span>
