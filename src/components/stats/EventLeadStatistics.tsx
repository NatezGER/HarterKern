  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10">
      <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] gap-3 bg-white/[0.04] px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-white/35 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
        <span>Spieler</span><span>Führungszeit</span><span>Übernommen</span>
        <span className="hidden sm:block">Verloren</span>
      </div>
      {data.map((row) => (
        <div key={row.playerId} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 border-t border-white/[0.06] px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
          ...
          <span className="text-center text-sm tabular-nums">{row.leadTakeovers}</span>
          <span className="hidden text-center text-sm tabular-nums sm:block">{row.leadLosses}</span>
          <span className="col-span-3 text-[10px] text-white/35 sm:hidden">{row.leadLosses}× Führung verloren</span>
        </div>
      ))}
    </div>
  );
