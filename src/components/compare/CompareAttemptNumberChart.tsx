  ...
  return (
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3 text-[10px] font-bold uppercase tracking-[0.12em] text-white/40">
        <span>Je höher, desto schneller</span>
  ...
  return (
    <div className="flex h-52 min-w-0 flex-col">
      <div className="grid min-h-0 flex-1 grid-cols-2 items-end gap-1 rounded-xl bg-black/15 px-1 pt-7">
        <AttemptBar point={playerA} playerName={playerAName} className="from-gold-700/70 to-gold-300" fastest={fastest} slowest={slowest} />
        <AttemptBar point={playerB} playerName={playerBName} className="from-cyan-800/70 to-cyan-300" fastest={fastest} slowest={slowest} />
      </div>
...
function AttemptBar({
  ...
  return (
    <button type="button" title={description} aria-label={description} className="group relative flex h-full min-w-0 items-end focus-visible:outline-none">
