...
import { Award, Crown, Gem, Medal, Shield, X } from "lucide-react";
...
export function BadgeUnlockCelebration() {
  ...
  const Icon = icons[badgeUnlock.tier];
  return (
    <motion.aside
      role="status"
      aria-live="assertive"
    ...
    >
      ...
      <motion.span animate={reduced ? undefined : { rotate: [0, -8, 8, 0], scale: [1, 1.12, 1] }} className="mx-auto grid size-16 place-items-center rounded-full border border-gold-400/30 bg-gold-400/10 text-gold-300"><Icon className="size-8" /></motion.span>
      <p className="mt-4 text-[10px] font-black uppercase tracking-[0.24em] text-gold-300">Badge freigeschaltet · {badgeUnlock.tier}</p>
