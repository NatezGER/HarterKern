...
export function EventResults({ event, attempts, players }: { event: LiveEvent; attempts: LiveAttempt[]; players: LiveParticipant[] }) {
  const eventAttempts = attempts.filter(({ eventId }) => eventId === event.id);
  const standings = getLiveStandings(event, eventAttempts, players);
  const podium = standings.filter(({ bestTime, player }) => bestTime != null && !player.isAk).slice(0, 3);
  ...
  return (
    <div className="space-y-7">
      <section className="panel overflow-hidden p-6 text-center sm:p-10">
        ...
        {winner && (
          <Link
            to={`/player/${winner.player.id}`}
            className="mx-auto mt-8 flex max-w-sm items-center justify-center gap-4 rounded-2xl bg-gold-400/[0.08] p-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400"
          >
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <ResultMetric icon={Target} label="Versuche" value={eventAttempts.length} />
        <ResultMetric icon={Flag} label="DNS" value={eventAttempts.filter(({ result }) => result === "dns").length} />
      </div>
      <section className="panel p-5 sm:p-7">
        <h2 className="display-title text-2xl">Top 3</h2>
        <div className="mt-5 space-y-2">
          {podium.map((standing, index) => (
            <Link to={`/player/${standing.player.id}`} key={standing.player.id} className="flex items-center gap-4 rounded-2xl bg-white/[0.03] p-4 transition hover:bg-white/[0.06]">
